...
function extractAssistantMessage(text) {
  const assistantTagStart = "<｜Assistant｜>";
  const endOfSentenceTag = "<｜end▁of▁sentence｜>";

  const assistantStartIndex = text.lastIndexOf(assistantTagStart);

  if (assistantStartIndex === -1) {
    return text;
  }

  const messageStartIndex = assistantStartIndex + assistantTagStart.length;
  const messageEndIndex = text.indexOf(endOfSentenceTag, messageStartIndex);

  if (messageEndIndex !== -1) {
    return text.substring(messageStartIndex, messageEndIndex).trim();
  } else {
    return text.substring(messageStartIndex).trim();
  }
}

function decodeOutput(tokenizer, outputs, skipSpecialTokens, returnThinking) {
  const decodedText = tokenizer.decode(outputs, {
    skip_special_tokens: skipSpecialTokens,
  });

  let outputText = skipSpecialTokens
    ? extractAssistantMessage(decodedText)
    : decodedText;

  if (!returnThinking) {
    outputText = removeThinkTags(outputText);
  }
    ...
    try {
      ...
      if (
        ...
        const outputText = decodeOutput(
          ...
          finalGenerationConfig.return_thinking,
        );
