...
// ==========================================
const CONFIG = {
  MODEL_ID: "distilroberta-finetuned-financial-news-sentiment-analysis",
...
class WebAIModel {
  ...
  async downloadModel(precision) {
    ...
    await pipeline("sentiment-analysis", this.model_id, {
      dtype: precision,
  ...
  async loadModel() {
    ...
    this.model = await pipeline("sentiment-analysis", this.model_id, {
      device: this.device,
  ...
  async generate(data) {
    ...
    try {
      ...
      for (const text of userInput.texts) {
        const response = await this.model(text, {
          ...finalModelConfig,
        });
