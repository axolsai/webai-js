const CONFIG = {
  ...
  MODEL_REVISION: "b3ca0d2f076785a8f4b2219ddbd2bdb99954eae1",
  MODEL_FILE: "gemma-4-E2B-it-web.litertlm",
  MODEL_BYTES: 2_008_432_640,
...
const MANIFEST = {
  ...
  model: {
    ...
    lastUpdated: "2026-07-20T16:41:56.000Z",
    sourceRepository: "google/gemma-4-E2B-it",
    repository: CONFIG.MODEL_REPOSITORY,
    artifactRevision: CONFIG.MODEL_REVISION,
    artifactLastUpdated: "2026-08-31T13:48:36.000Z",
    task: "text-generation",
    description: "Gemma 4 E2B Instruct is a compact Google language model that performs text-only conversational generation locally in a WebGPU-capable browser through LiteRT-LM.",
    intendedUses: [
    ...
    limitations: [
      ...
      "The approximately 2 GB initial download and roughly 1.8 GB reported GPU-memory use require a capable desktop-class device.",
  ...
  benchmark: {
    report: "benchmarks/gemma-4-e2b-it-litert-lm.json",
    testedAt: "2026-09-09T04:57:20.952Z",
    fixture: {
      file: "gemma-4-e2b-it-litert-lm.json",
      url: "https://f005.backblazeb2.com/file/hubters-webai-public/assets/benchmarks/gemma-4-e2b-it-litert-lm.json",
      size: 195,
    ...
    results: [{
      ...
      device: "webgpu",
      status: "pass",
      runtime: { loadMs: 124226.3, inferenceMs: 206.2, timeToFirstTokenMs: 166, prefillTokensPerSecond: 152.419, decodeTokensPerSecond: 66.225, totalMs: 124432.5, grade: "excellent" },
...
function validateInit(data) {
  if (data?.precision !== "q4") throw new Error("Gemma 4 E2B LiteRT-LM supports only the WebAI q4 selection.");
  if (data?.device !== "webgpu") throw new Error("Gemma 4 E2B LiteRT-LM requires WebGPU.");
